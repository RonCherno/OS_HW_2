...
/* $end Pthread_Create wrapper */

/* $begin Pthread_Join wrapper */
int Pthread_Join(pthread_t thread, void **thread_return){
    int return_value = pthread_join(thread, thread_return);
    if (return_value != 0){
        unix_error("pthread_join error");
    }
    return return_value;
}
/* $end Pthread_Join wrapper */
